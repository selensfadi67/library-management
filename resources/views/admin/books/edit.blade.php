...
@section('header', __('messages.edit_book'))
...
<div class="kt-section__content">
    <div class="kt-portlet">
        <div class="kt-portlet__head">
            <div class="kt-portlet__head-label">
                <h3 class="kt-portlet__head-title">
                    {{ __('messages.edit_book') ?? 'Edit Book' }}: {{ $book->title }}
                </h3>
            </div>
        ...
        <div class="kt-portlet__body">
            <form action="{{ route('admin.books.update', [$lang, $book]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-md-8">
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control @error('release_date') is-invalid @enderror" 
                                   id="release_date" name="release_date" value="{{ old('release_date', $book->release_date?->format('Y-m-d')) }}">
                            @error('release_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <small class="form-text text-muted">
                                {{ __('messages.image_help') ?? 'Leave empty to keep current image' }}
                            </small>
                        </div>
                        ...
                        <div class="form-group">
                            <div class="card">
                                <div class="card-body text-center">
                                    @if($book->image)
                                        <img src="{{ asset('storage/' . $book->image) }}" class="img-fluid" style="max-height: 200px;" alt="{{ $book->title }}">
                                        <p class="text-muted mt-2">{{ __('messages.current_image') ?? 'Current Image' }}</p>
                                    @else
                                        <i class="la la-image la-5x text-muted mb-3"></i>
                                        <p class="text-muted">{{ __('messages.no_image') ?? 'No image uploaded' }}</p>
                                    @endif
                                </div>
                            </div>
                        </div>
                ...
                <div class="form-group">
                    <button type="submit" class="btn btn-primary">
                        <i class="la la-save"></i> {{ __('messages.update') ?? 'Update Book' }}
                    </button>
                    <a href="{{ route('admin.books.index', $lang) }}" class="btn btn-secondary">
